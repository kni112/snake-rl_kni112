...
# Define a class for the Torch DQN model, inheriting from torch.nn.Module
class DQN(nn.Module):
    def __init__(self, version: str, board_size: int, n_frames: int, n_actions:int):
        super().__init__()
        ...
        with open(f"model_config/{version}.json", "r") as file:
            config = json.load(file)
            model_config = config["model"]

        # Building the CNN dynamically from configuration file like the original repository
        # Populate a list of layers, define parameters
        conv_layers = []
        ...
        # Iterate over model specifications from config file to populate layers list
        for name, layer_config in model_config.items():
            ...
            if "Conv2D" in name:
                ...
                padding = 0
                if layer_config.get("padding") == "same": 
                    padding = kernel_size[0] // 2 # Calculate padding for torch manually (odd kernel sizes)
                ...
                # Add activation function for layer if specified
                if layer_config.get("activation") == "relu":
                    conv_layers.append(nn.ReLU(inplace=True))

                in_channels = out_channels # Update input channels for next layer

            # Enable flatten, if specified
            elif "Flatten" in name:
                use_flatten = True

            # Enable dense (fully connected) layer before output, if specified
            elif "Dense" in name:
                dense_units = layer_config["units"]
                dense_activation = layer_config.get("activation", None)
        
        self.conv = nn.Sequential(*conv_layers) # Unpack list of layers to pass to the sequential module
        
        self.flatten = nn.Flatten() if use_flatten else nn.Identity()

        # Calculate dimension after flattening
        with torch.no_grad():
            dummy = torch.zeros(1, n_frames, board_size, board_size)
            x = self.conv(dummy)
            x = self.flatten(x)
            flat_dim = x.shape[1]
        ...
        self.fc1_activation = nn.ReLU(inplace=True) if dense_activation == "relu" else nn.Identity()
    ...
    def forward(self, x):
        ...
        x = self.flatten(x)         # Flatten dimensions
        x = self.fc1(x)             # Fully connected layer
        x = self.fc1_activation(x)  # Activation
        return self.out(x)
...
class DeepQLearningAgent(Agent):
    ...
    def __init__(self, board_size=10, frames=4, buffer_size=10000,
        ...
        # Choose device -> Compute with Cuda if available
